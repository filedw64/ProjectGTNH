...
public class SyncEmcPKT implements IMessage
{
	...
	public void fromBytes(ByteBuf buf)
	{
		...
		for (int i = 0; i < size; i++)
		{
			...
			if (arraylen == 5) {
				// 新增：读取 NBT 数据
				array[4] = ByteBufUtils.readTag(buf);
			} else if (arraylen == 6) {
	...
	public void toBytes(ByteBuf buf)
	{
		...
		for (Object obj : data)
		{
			...
			if (array.length == 5) {
				// 写入 NBT
				ByteBufUtils.writeTag(buf, (NBTTagCompound) array[4]);
	...
	public static class Handler implements IMessageHandler<SyncEmcPKT, IMessage>
	{
		...
		public IMessage onMessage(final SyncEmcPKT pkt, MessageContext ctx)
		{
			...
			for (Object obj : pkt.data)
			{
				...
				if (array.length == 5) {
					// 还原带有 NBT 的 SimpleStack
					stack = new SimpleStack((int) array[0], (int) array[1], (int) array[2], (NBTTagCompound) array[4]);
				} else if (array.length == 6) {
					stack = new GTSimpleStack((int) array[0], (int) array[1], (int) array[2], (String) array[4], (String) array[5]);
				} else {
					stack = new SimpleStack((int) array[0], (int) array[1], (int) array[2]);
				}
