...
public final class PlayerChecks
{
	...
	public static void setGemState(EntityPlayerMP player, boolean state) {
		if (state) gemArmorReadyChecks.add(player);
		else gemArmorReadyChecks.remove(player);
	}
	...
	private static boolean shouldPlayerFly(EntityPlayerMP player)
	{
		...
		for (ItemStack stack : player.inventory.armorInventory) {
			if (stack != null && stack.getItem() instanceof IFlightProvider ifp && ifp.canProvideFlight(stack, player))
				return true;
		}
	...
	private static boolean shouldPlayerResistFire(EntityPlayerMP player) {
		...
		for (ItemStack stack : player.inventory.armorInventory) {
			if (stack != null && stack.getItem() instanceof IFireProtector ifr && ifr.canProtectAgainstFire(stack, player))
				return true;
		}
	...
	private static boolean shouldPlayerStep(EntityPlayerMP player) {
		for (ItemStack stack : player.inventory.armorInventory) {
			if (stack != null && stack.getItem() instanceof IStepAssister isa && isa.canAssistStep(stack, player))
				return true;
		}
