...
public class RMFurnaceContainer extends Container
{
	...
	public RMFurnaceContainer(InventoryPlayer invPlayer, RMFurnaceTile tile)
	{
		...
		//Fuel
		this.addSlotToContainer(new SlotFurnaceFuel(tile, 0, 65, 53));

		//Input(0)
		this.addSlotToContainer(new SlotFurnaceInput(tile, 1, 65, 17));

		//Input storage
		for (int i = 0; i < 3; i++)
			for (int j = 0; j < 4; j++)
				this.addSlotToContainer(new SlotFurnaceInput(tile, i * 4 + j + 2, 11 + i * 18, 8 + j * 18));

		//Output(0)
		this.addSlotToContainer(new SlotFurnaceOutput(tile, 14, 125, 35));

		//Output Storage
		for (int i = 0; i < 3; i++)
			for (int j = 0; j < 4; j++)
				this.addSlotToContainer(new SlotFurnaceOutput(tile, i * 4 + j + 15, 147 + i * 18, 8 + j * 18));
