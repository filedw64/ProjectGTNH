...
public abstract class PEToolBase extends ItemMode
{
	...
	protected void clearOdAOE(World world, ItemStack stack, EntityPlayer player, String odName, int emcCost)
	{
		...
		if (world.isRemote || charge == 0 || ProjectEConfig.disableAllRadiusMining) return;

		List<ItemStack> drops = new ArrayList<>();

		final int minX = (int) player.posX - (5 * charge), maxX = (int) player.posX + (5 * charge);
		final int minY = (int) player.posY - (10 * charge), maxY = (int) player.posY + (10 * charge);
		final int minZ = (int) player.posZ - (5 * charge), maxZ = (int) player.posZ + (5 * charge);
		...
		for (int x = minX; x <= maxX; x++) {
			for (int z = minZ; z <= maxZ; z++)
			{
				...
				for (int y = minY; y <= maxY; y++) {
					...
					else oreName = OreDictionary.getOreName(oreIds[0]);
						...
						&& consumeFuel(player, stack, emcCost, true))
					{
						...
						if (world.rand.nextInt(5) == 0) {
							PacketHandler.sendToAllAround(new ParticlePKT("largesmoke", x, y, z),
	...
	private final static Block.SoundType tillSound = Blocks.farmland.stepSound;
	...
	protected void tillAOE(ItemStack stack, EntityPlayer player, World world, int x, int y, int z, int meta, int emcCost)
	{
		...
		for (int i = x - charge; i <= x + charge; i++) {
			for (int j = z - charge; j <= z + charge; j++)
			{
				...
				if (blockAbove.isOpaqueCube() || (block != Blocks.grass && block != Blocks.dirt)) continue;

				if (!hasSoundPlayed) {
					world.playSoundEffect(i + 0.5D, y + 0.5D, j + 0.5D, tillSound.getStepResourcePath(),
						(tillSound.getVolume() + 1.0F) / 2.0F, tillSound.getPitch() * 0.8F);
					hasSoundPlayed = true;
				}
	...
	{
		if (world.isRemote || !(living instanceof EntityPlayer player)) return;
	...
	protected void attackWithCharge(ItemStack stack, EntityLivingBase damaged, EntityLivingBase damager, float baseDmg) {
		if (damager.worldObj.isRemote || !(damager instanceof EntityPlayer player)) return;
	...
	protected void attackAOE(ItemStack stack, EntityPlayer player, boolean slayAll, float damage, int emcCost) {
		...
		for (Entity entity : toAttack) {
			if (!consumeFuel(player, stack, emcCost, true)) continue;

			if (entity instanceof IMob || (entity instanceof EntityLivingBase && slayAll)) {
				entity.attackEntityFrom(src, damage);
				entity.hurtResistantTime = 0; // 清除无敌帧
			}
		}
		player.worldObj.playSoundAtEntity(player, "projecte:item.pecharge", 0.5F, 1.0F);
		PlayerHelper.swingItem(player);
	}
	...
	protected void shearBlock(ItemStack stack, int x, int y, int z, EntityPlayer player)
	{
		...
		if (!(block instanceof IShearable target)) return;
	...
	protected void shearEntityAOE(ItemStack stack, EntityPlayer player, int emcCost)
	{
		...
		for (EntityLivingBase ent : list)
		{
			...
			if (target.isShearable(stack, ent.worldObj, (int) ent.posX, (int) ent.posY, (int) ent.posZ)
				&& consumeFuel(player, stack, emcCost, true))
			{
				ArrayList<ItemStack> entDrops = target.onSheared(stack, ent.worldObj, (int) ent.posX, (int) ent.posY,
					(int) ent.posZ, EnchantmentHelper.getEnchantmentLevel(Enchantment.fortune.effectId, stack));
	...
	protected void tryVeinMine(ItemStack stack, EntityPlayer player, MovingObjectPosition mop)
	{
		...
		AxisAlignedBB aabb = WorldHelper.getBroadDeepBox(new Coordinates(mop.blockX, mop.blockY, mop.blockZ),
			ForgeDirection.getOrientation(mop.sideHit), getCharge(stack));
		...
		if (target.getBlockHardness(player.worldObj, mop.blockX, mop.blockY, mop.blockZ) <= -1 || !(canHarvestBlock(target, stack)
			|| ForgeHooks.canToolHarvestBlock(target, player.worldObj.getBlockMetadata(mop.blockX, mop.blockY, mop.blockZ), stack)))
			return;

		List<ItemStack> drops = new ArrayList<>();

		final Coordinates coord = new Coordinates(0, 0, 0); // 复用对象，减少创建
		for (int i = (int) aabb.minX; i <= aabb.maxX; i++) {
			for (int k = (int) aabb.minZ; k <= aabb.maxZ; k++)
			{
				if (!player.worldObj.blockExists(i, 0, k))
					continue;
				for (int j = (int) aabb.minY; j <= aabb.maxY; j++) {
					...
					coord.y = j;
					coord.z = k;
					WorldHelper.harvestVein(player.worldObj, player, stack, coord, b, drops, 0);
				}
			}
		}
		...
		for (int x = (int) box.minX; x <= box.maxX; x++) {
			for (int z = (int) box.minZ; z <= box.maxZ; z++)
			{
				...
				for (int y = (int) box.minY; y <= box.maxY; y++) {
					Block block = world.getBlock(x, y, z);
					if (ItemHelper.isOre(block, world.getBlockMetadata(x, y, z)) && block.getBlockHardness(world, x, y, z) != -1
						&& (canHarvestBlock(block, stack) || ForgeHooks.canToolHarvestBlock(block, world.getBlockMetadata(x, y, z), stack)))
					{
						coord.x = x;
						coord.y = y;
						coord.z = z;
						WorldHelper.harvestVein(world, player, stack, coord, block, drops, 0);
					}
				}
			}
		}
