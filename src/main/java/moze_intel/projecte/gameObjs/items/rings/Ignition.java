...
public class Ignition extends RingToggle implements IBauble, IPedestalItem, IFireProtector, IProjectileShooter
{
	...
	public void onUpdate(ItemStack stack, World world, Entity entity, int inventorySlot, boolean par5) {
		if (world.isRemote || inventorySlot > 8 || !(entity instanceof EntityPlayerMP player)) return;
		...
		if (stack.getItemDamage() != 0)
		{
			if (getEmc(stack) == 0 && !consumeFuel(player, stack, 64, false))
				...
				for (EntityLivingBase ent : entities)
					if (ent instanceof IMob && !ent.isBurning())
						ent.setFire(3);

				removeEmc(stack, 0.32F);
			}
		}
		else WorldHelper.extinguishNearby(world, player);
	}
	...
	public void changeMode(EntityPlayer player, ItemStack stack) {
		if (stack.getItemDamage() != 0)
			stack.setItemDamage(0);
		else if (getEmc(stack) != 0 || consumeFuel(player, stack, 64, false))
	...
	public ItemStack onItemRightClick(ItemStack stack, World world, EntityPlayer player)
	{
		if (!world.isRemote && player instanceof EntityPlayerMP entityPlayerMP)
		{
			...
			{
				if (world.getBlock(mop.blockX, mop.blockY, mop.blockZ) instanceof BlockTNT tnt
					&& PlayerHelper.hasBreakPermission(entityPlayerMP, mop.blockX, mop.blockY, mop.blockZ))
				{
					tnt.func_150114_a(world, mop.blockX, mop.blockY, mop.blockZ, 1, player);
	...
	{
		if (world.isRemote || ProjectEConfig.ignitePedCooldown == -1) return;
	...
	public List<String> getPedestalDescription()
	{
		...
		if (ProjectEConfig.ignitePedCooldown != -1) {
			...
			list.add(EnumChatFormatting.BLUE + String.format(StatCollector.translateToLocal("pe.ignition.pedestal2"),
