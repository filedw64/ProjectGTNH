...
public class SWRG extends ItemPE implements IBauble, IPedestalItem, IFlightProvider
{
	...
	{
		if (stack.getItemDamage() > 1) // Repel on both sides - smooth animation
	...
	{
		if (invSlot > 8 || !(entity instanceof EntityPlayer entityPlayer))
			return;
		tick(stack, entityPlayer);
	}
	...
	{
		if (!(ent instanceof EntityPlayer entityPlayer))
			return;
		tick(stack, entityPlayer);
	}
	...
	public void updateInPedestal(World world, int x, int y, int z) {
		if (world.isRemote || ProjectEConfig.swrgPedCooldown == -1) return;
	...
	public List<String> getPedestalDescription()
	{
		...
		if (ProjectEConfig.swrgPedCooldown != -1) {
			...
			list.add(EnumChatFormatting.BLUE + String.format(StatCollector.translateToLocal("pe.swrg.pedestal2"),
