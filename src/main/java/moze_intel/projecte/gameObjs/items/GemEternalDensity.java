...
public class GemEternalDensity extends ItemPE implements IAlchBagItem, IAlchChestItem, IModeChanger, IBauble
{
	...
	public void onUpdate(ItemStack stack, World world, Entity entity, int slot, boolean isHeld) {
		...
		if (world.getTotalWorldTime() % 20 == 0)
			condense(stack, player.inventory.mainInventory);
	}
	...
	public static boolean condense(ItemStack gem, ItemStack[] inv)
	{
		...
		for (int i = 0; i < inv.length; i++) {
			...
			if (!EMCHelper.doesItemHaveEmc(s) || s.getMaxStackSize() == 1 || (emc = EMCHelper.getEmcValue(s)) >= targetEmc)
				continue;
	...
	public ItemStack onItemRightClick(ItemStack stack, World world, EntityPlayer player)
	{
		...
		if (player.isSneaking()) {
			...
			player.openGui(PECore.instance, Constants.ETERNAL_DENSITY_GUI, world, (int) player.posX,
	...
	private static ItemStack getTarget(ItemStack stack)
	{
		return switch (stack.stackTagCompound.getByte("Target")) {
			...
			default -> {
				PELogger.logError("Invalid target for gem of eternal density: " + stack.stackTagCompound.getByte("Target"));
				yield null;
			}
		};
	}
	...
	private static void addToList(List<ItemStack> list, ItemStack stack) {
		...
		for (ItemStack s : list) {
			if (s.stackSize >= s.getMaxStackSize() || !ItemHelper.areItemStacksEqual(s, stack)) continue;
	...
	public void changeMode(EntityPlayer player, ItemStack stack) {
		...
		else stack.stackTagCompound.setByte("Target", (byte) (oldMode + 1));
	...
	public void addInformation(ItemStack stack, EntityPlayer player, List<String> list, boolean par4)
	{
		...
		if (stack.hasTagCompound()) {
			list.add(String.format(StatCollector.translateToLocal("pe.gemdensity.tooltip2"),
				StatCollector.translateToLocal(getTargetName(stack))));
		}

		list.add(String.format(StatCollector.translateToLocal("pe.gemdensity.tooltip3"),
	...
	public void updateInAlchChest(World world, int x, int y, int z, ItemStack stack) {
		...
		if (world.isRemote || stack.getItemDamage() != 1 || world.getTotalWorldTime() % 20 != 0) return;
