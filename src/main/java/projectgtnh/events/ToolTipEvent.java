...
public class ToolTipEvent 
{
	...
	public void tTipEvent(ItemTooltipEvent event)
	{
		...
		if (ProjectGTNHConfig.showPedestalTooltip
			&& currentItem instanceof IPedestalItem)
		{
			if (ProjectGTNHConfig.showPedestalTooltipInGUI)
			{
		...
		if (ProjectGTNHConfig.showUnlocalizedNames)
		{
			event.toolTip.add("UN: " + Item.itemRegistry.getNameForObject(current.getItem()));
		}
		
		if (ProjectGTNHConfig.showODNames)
		{
		...
		if (ProjectGTNHConfig.showEMCTooltip)
		{
		...
		if (ProjectGTNHConfig.showStatTooltip)
		{
			/**
			 * Collector ToolTips
			 */
		...
		{
			if (current.stackTagCompound.getBoolean("ProjectGTNHBlock"))
			{
