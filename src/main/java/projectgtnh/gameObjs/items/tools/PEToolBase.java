package projectgtnh.gameObjs.items.tools;
...
public abstract class PEToolBase extends ItemMode
{
	...
	protected void clearOdAOE(World world, ItemStack stack, EntityPlayer player, String odName, int emcCost)
	{
		...
		if (charge == 0 || world.isRemote || ProjectGTNHConfig.disableAllRadiusMining)
		{
			return;
		}
	...
	protected void digBasedOnMode(ItemStack stack, World world, Block block, int x, int y, int z, EntityLivingBase living)
	{
		...
		if (ProjectGTNHConfig.disableAllRadiusMining) {
	...
	{
		if (world.isRemote || this.getCharge(stack) == 0 || ProjectGTNHConfig.disableAllRadiusMining)
		{
			return;
		}
	...
	{
		if (player.worldObj.isRemote || ProjectGTNHConfig.disableAllRadiusMining)
		{
			return;
		}
