...
public class TimeWatch extends ItemCharge implements IModeChanger, IBauble, IPedestalItem
{
	...
	public ItemStack onItemRightClick(ItemStack stack, World world, EntityPlayer player)
	{
		if (!world.isRemote)
		{
			if (!ProjectGTNHConfig.enableTimeWatch)
			{
	...
	public void updateInPedestal(World world, int x, int y, int z)
	{
		...
		if (!world.isRemote && ProjectGTNHConfig.enableTimeWatch)
		{
			AxisAlignedBB bBox = ((DMPedestalTile) world.getTileEntity(x, y, z)).getEffectBounds();
			if (ProjectGTNHConfig.timePedBonus > 0) {
				speedUpTileEntities(world, ProjectGTNHConfig.timePedBonus, bBox);
				speedUpRandomTicks(world, ProjectGTNHConfig.timePedBonus, bBox);
			}

			if (ProjectGTNHConfig.timePedMobSlowness < 1.0F) {
				slowMobs(world, bBox, ProjectGTNHConfig.timePedMobSlowness);
			}
		}
	}
	...
	public List<String> getPedestalDescription()
	{
		...
		if (ProjectGTNHConfig.timePedBonus > 0) {
			list.add(EnumChatFormatting.BLUE +
				String.format(StatCollector.translateToLocal("pe.timewatch.pedestal1"), ProjectGTNHConfig.timePedBonus));
		}
		if (ProjectGTNHConfig.timePedMobSlowness < 1.0F)
		{
			list.add(EnumChatFormatting.BLUE +
					String.format(StatCollector.translateToLocal("pe.timewatch.pedestal2"), ProjectGTNHConfig.timePedMobSlowness));
		}
		return list;
	}
