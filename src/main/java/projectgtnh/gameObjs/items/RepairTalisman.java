...
import projectgtnh.api.item.IAlchBagItem;
import projectgtnh.api.item.IAlchChestItem;
...
import projectgtnh.gameObjs.items.rings.RingToggle;
...
public class RepairTalisman extends ItemPE implements IAlchBagItem, IAlchChestItem, IBauble, IPedestalItem
{
	...
	{
		if (!world.isRemote && ProjectGTNHConfig.repairPedCooldown != -1)
		{
			...
			if (tile.getActivityCooldown() == 0)
			{
				...
				tile.setActivityCooldown(ProjectGTNHConfig.repairPedCooldown);
			}
			else
			{
	...
	public List<String> getPedestalDescription()
	{
		...
		if (ProjectGTNHConfig.repairPedCooldown != -1)
		{
			...
			list.add(EnumChatFormatting.BLUE +
					String.format(StatCollector.translateToLocal("pe.repairtalisman.pedestal2"), MathUtils.tickToSecFormatted(ProjectGTNHConfig.repairPedCooldown)));
		}
		return list;
	}
