...
public class EvertideAmulet extends ItemPE implements IProjectileShooter, IBauble, IPedestalItem, IFluidContainerItem
{
	...
	{
		if (!world.isRemote && ProjectGTNHConfig.evertidePedCooldown != -1)
		{
			...
			if (tile.getActivityCooldown() == 0)
			{
				...
				tile.setActivityCooldown(ProjectGTNHConfig.evertidePedCooldown);
			}
			else
			{
	...
	public List<String> getPedestalDescription()
	{
		...
		if (ProjectGTNHConfig.evertidePedCooldown != -1)
		{
			list.add(EnumChatFormatting.BLUE + StatCollector.translateToLocal("pe.evertide.pedestal1"));
			list.add(EnumChatFormatting.BLUE + String.format(
					StatCollector.translateToLocal("pe.evertide.pedestal2"), MathUtils.tickToSecFormatted(ProjectGTNHConfig.evertidePedCooldown)));
		}
		return list;
	}
}
