...
public class BodyStone extends RingToggle implements IBauble, IPedestalItem
{
	...
	{
		if (!world.isRemote && ProjectGTNHConfig.bodyPedCooldown != -1)
		{
			...
			if (tile.getActivityCooldown() == 0)
			{
				...
				tile.setActivityCooldown(ProjectGTNHConfig.bodyPedCooldown);
			}
			else
			{
	...
	public List<String> getPedestalDescription()
	{
		...
		if (ProjectGTNHConfig.bodyPedCooldown != -1)
		{
			list.add(EnumChatFormatting.BLUE + StatCollector.translateToLocal("pe.body.pedestal1"));
			list.add(EnumChatFormatting.BLUE + String.format(
					StatCollector.translateToLocal("pe.body.pedestal2"), MathUtils.tickToSecFormatted(ProjectGTNHConfig.bodyPedCooldown)));
		}
		return list;
	}
}
