...
public class HarvestGoddess extends RingToggle implements IPedestalItem
{
	...
	{
		if (!world.isRemote && ProjectGTNHConfig.harvestPedCooldown != -1)
		{
			DMPedestalTile tile = (DMPedestalTile) world.getTileEntity(x, y, z);
			if (tile.getActivityCooldown() == 0)
			{
				WorldHelper.growNearbyRandomly(true, world, x, y, z, null);
				tile.setActivityCooldown(ProjectGTNHConfig.harvestPedCooldown);
			}
			else
			{
	...
	public List<String> getPedestalDescription()
	{
		...
		if (ProjectGTNHConfig.harvestPedCooldown != -1)
		{
			...
			list.add(EnumChatFormatting.BLUE + String.format(
					StatCollector.translateToLocal("pe.harvestgod.pedestal3"), MathUtils.tickToSecFormatted(ProjectGTNHConfig.harvestPedCooldown)));
		}
		return list;
	}
