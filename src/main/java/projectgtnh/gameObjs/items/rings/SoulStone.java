...
public class SoulStone extends RingToggle implements IBauble, IPedestalItem
{
	...
	{
		if (!world.isRemote && ProjectGTNHConfig.soulPedCooldown != -1)
		{
			...
			if (tile.getActivityCooldown() == 0)
			{
				...
				tile.setActivityCooldown(ProjectGTNHConfig.soulPedCooldown);
			}
			else
			{
	...
	public List<String> getPedestalDescription()
	{
		...
		if (ProjectGTNHConfig.soulPedCooldown != -1)
		{
			list.add(EnumChatFormatting.BLUE + StatCollector.translateToLocal("pe.soul.pedestal1"));
			list.add(EnumChatFormatting.BLUE + String.format(
					StatCollector.translateToLocal("pe.soul.pedestal2"), MathUtils.tickToSecFormatted(ProjectGTNHConfig.soulPedCooldown)));
		}
		return list;
	}
}
