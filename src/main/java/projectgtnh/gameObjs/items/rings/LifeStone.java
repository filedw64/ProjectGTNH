...
public class LifeStone extends RingToggle implements IBauble, IPedestalItem
{
	...
	{
		if (!world.isRemote && ProjectGTNHConfig.lifePedCooldown != -1)
		{
			...
			if (tile.getActivityCooldown() == 0)
			{
				...
				tile.setActivityCooldown(ProjectGTNHConfig.lifePedCooldown);
			}
			else
			{
	...
	public List<String> getPedestalDescription()
	{
		...
		if (ProjectGTNHConfig.lifePedCooldown != -1)
		{
			list.add(EnumChatFormatting.BLUE + StatCollector.translateToLocal("pe.life.pedestal1"));
			list.add(EnumChatFormatting.BLUE + String.format(
					StatCollector.translateToLocal("pe.life.pedestal2"), MathUtils.tickToSecFormatted(ProjectGTNHConfig.lifePedCooldown)));
		}
		return list;
	}
}
