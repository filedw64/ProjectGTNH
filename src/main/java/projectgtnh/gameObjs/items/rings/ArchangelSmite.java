...
public class ArchangelSmite extends RingToggle implements IPedestalItem, IModeChanger
{
	...
	{
		if (!world.isRemote && ProjectGTNHConfig.archangelPedCooldown != -1)
		{
			...
			if (tile.getActivityCooldown() == 0)
			{
				...
				tile.setActivityCooldown(ProjectGTNHConfig.archangelPedCooldown);
			}
			else
			{
	...
	public List<String> getPedestalDescription()
	{
		...
		if (ProjectGTNHConfig.archangelPedCooldown != -1) {
			list.add(EnumChatFormatting.BLUE + StatCollector.translateToLocal("pe.archangel.pedestal1"));
			list.add(EnumChatFormatting.BLUE + String.format(
					StatCollector.translateToLocal("pe.archangel.pedestal2"), MathUtils.tickToSecFormatted(ProjectGTNHConfig.archangelPedCooldown)));
		}
		return list;
	}
}
