...
import projectgtnh.gameObjs.items.ItemCharge;
...
public class Zero extends ItemCharge implements IModeChanger, IBauble, IPedestalItem
{
	...
	{
		if (!world.isRemote && ProjectGTNHConfig.zeroPedCooldown != -1)
		{
			...
			if (tile.getActivityCooldown() == 0) {
				...
				tile.setActivityCooldown(ProjectGTNHConfig.zeroPedCooldown);
			}
			else
			{
	...
	public List<String> getPedestalDescription()
	{
		...
		if (ProjectGTNHConfig.zeroPedCooldown != -1) {
			...
			list.add(EnumChatFormatting.BLUE + String.format(
					StatCollector.translateToLocal("pe.zero.pedestal3"), MathUtils.tickToSecFormatted(ProjectGTNHConfig.zeroPedCooldown)));
		}
		return list;
	}
}
