...
import projectgtnh.gameObjs.items.IFlightProvider;
...
public class SWRG extends ItemPE implements IBauble, IPedestalItem, IFlightProvider
{
	...
	{
		if (!world.isRemote && ProjectGTNHConfig.swrgPedCooldown != -1)
		{
			...
			if (tile.getActivityCooldown() <= 0)
			{
				...
				tile.setActivityCooldown(ProjectGTNHConfig.swrgPedCooldown);
			}
			else
			{
	...
	public List<String> getPedestalDescription()
	{
		...
		if (ProjectGTNHConfig.swrgPedCooldown != -1)
		{
			list.add(EnumChatFormatting.BLUE + StatCollector.translateToLocal("pe.swrg.pedestal1"));
			list.add(EnumChatFormatting.BLUE + String.format(
					StatCollector.translateToLocal("pe.swrg.pedestal2"), MathUtils.tickToSecFormatted(ProjectGTNHConfig.swrgPedCooldown)));
		}
		return list;
	}
}
